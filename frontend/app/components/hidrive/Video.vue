<template>
  <main class="min-h-screen bg-neutral-200 text-black">
    <section class="sticky top-0 w-screen">
      ...
      <nav
        v-if="true"
        class="flex items-center justify-between p-4 backdrop-blur focus:outline-none"
      >
        <nuxt-link
          class="flex h-12 w-12 items-center text-white"
          to="/videos"
        >
    ...
    <section class="mx-auto max-w-screen-md bg-neutral-200 pb-16 pt-8">
      <h2 class="text-md px-4 py-1 font-bold text-neutral-600">
        Kapitel:
      </h2>
...
const video = computed(() => {
  return {
    src: 'http://localhost:8000/hi/media/' + props.meta.path,
...
onMounted(() => {
  ...
  videoElem.value.onplaying = function () {
