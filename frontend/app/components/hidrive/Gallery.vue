<template>
  ...
  >
    <a
      v-for="f in files"
      :key="f.id"
      :href="'http://localhost:8000/hi/media' + dir + '/' + f.name"
    ...
    >
      <img
        :src="'http://localhost:8000/hi/media/thumbs?path=' + dir + '/' + f.name + '&width=200'"
        :alt="f.name"
...
const props = defineProps(['images', 'gallery', 'dir'])
