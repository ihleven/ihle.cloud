<template>
  <section class="bg-green-500 p-2">
    <div class="flex items-start justify-between">
      <div class="left">
        <div class="text-outline flex">
          <span class="flex flex-col items-end justify-between p-2">
            <nuxt-link
            ...
            <nuxt-link class="text-3xl font-black" :to="'/kalender/' + jahr">{{ jahr }}</nuxt-link>
          </span>
          <span v-show="tag" class="text-8xl font-black leading-[6rem]">{{ tag }}</span>
        </div>
...
<script setup>
  ...
  const kw = 'tag'
  const monatsname = computed(() => date.value.toLocaleDateString('de-DE', { month: 'long' }))
  const wochentag = computed(() => date.value.toLocaleDateString('de-DE', { weekday: 'long' }))
