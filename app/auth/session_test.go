...
func signedInAccount(t *testing.T, svc *Service, ctx context.Context, name, plain string) (*Account, *http.Cookie) {
	t.Helper()
...
func postLogin(svc *Service, username, password string) *httptest.ResponseRecorder {
	...
	if err := svc.Login(w, r); err != nil {
		// Stands in for the router's error middleware, which is what turns a
		// returned error into a response.
		http.Error(w, err.Error(), errs.StatusOf(err))
	}
	return w
}
