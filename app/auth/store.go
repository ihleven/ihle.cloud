...
func (s *Store) CreateAccount(ctx context.Context, name, displayName, email string) (*Account, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("auth: creating account %s: %w", name, err)
	}
	return &Account{
...
func (s *Store) load(ctx context.Context, where string, arg any) (*Account, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("auth: loading account: %w", err)
	}
	return a, nil
}
...
func (s *Store) SetDisabled(ctx context.Context, name string, disabled bool) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("auth: setting disabled on %s: %w", name, err)
	}
	return id, nil
}
...
func (s *Store) SetIdentity(ctx context.Context, id int64, displayName, email string) error {
	...
	if err != nil {
		return fmt.Errorf("auth: setting identity for account %d: %w", id, err)
	}
...
func (s *Store) SetPasswordHash(ctx context.Context, id int64, hash string) error {
	...
	if err != nil {
		return fmt.Errorf("auth: setting password for account %d: %w", id, err)
	}
	if tag.RowsAffected() == 0 {
		return ErrNoAccount
	}
...
func (s *Store) SetCMSProfile(ctx context.Context, id int64, p CMSProfile) error {
	...
	if err != nil {
		return fmt.Errorf("auth: setting cms profile for account %d: %w", id, err)
	}
	return nil
}
...
// PasskeyInfo is a credential as it is shown to a person: enough to recognise a
// device by and decide whether it is still theirs, and nothing of the credential
// itself, which would mean nothing to anyone.
//
// One type rather than one per caller, because the self-service page and the
// admin section show the same thing about the same rows — they differ in whose
// devices they may ask for, which is a question of authorisation, not of shape.
...
// Info projects a stored credential for display. The id is base64url because it
// travels in a URL when a device is removed.
func (p StoredPasskey) Info() PasskeyInfo {
	return PasskeyInfo{
		ID:         base64.RawURLEncoding.EncodeToString(p.ID),
		Name:       p.Name,
		Syncable:   p.BackupEligible,
...
func (s *Store) Passkeys(ctx context.Context, accountID int64) ([]webauthn.Credential, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("auth: loading passkeys: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := json.Unmarshal(raw, &c); err != nil {
			return nil, fmt.Errorf("auth: decoding passkey: %w", err)
		}
...
func (s *Store) ListPasskeys(ctx context.Context, accountID int64) ([]StoredPasskey, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("auth: listing passkeys: %w", err)
	}
	defer rows.Close()
...
func (s *Store) AddPasskey(ctx context.Context, accountID int64, cred *webauthn.Credential, name string) error {
	...
	if err != nil {
		return fmt.Errorf("auth: encoding passkey: %w", err)
	}
	_, err = s.pool.Exec(ctx, `
	...
	if err != nil {
		return fmt.Errorf("auth: storing passkey: %w", err)
	}
	return nil
}
...
func (s *Store) UpdatePasskey(ctx context.Context, cred *webauthn.Credential) error {
	...
	if err != nil {
		return fmt.Errorf("auth: updating passkey: %w", err)
	}
	return nil
}
...
func (s *Store) DeletePasskey(ctx context.Context, accountID int64, id []byte) error {
	...
	if err != nil {
		return fmt.Errorf("auth: deleting passkey: %w", err)
	}
	if tag.RowsAffected() == 0 {
		return ErrNoAccount
	}
...
func (s *Store) DeletePasskeys(ctx context.Context, accountID int64) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("auth: deleting passkeys: %w", err)
	}
...
func (s *Store) CreateEnrollToken(ctx context.Context, accountID int64, ttl time.Duration) (string, error) {
	...
	if _, err := s.pool.Exec(ctx,
		...
		return "", fmt.Errorf("auth: creating enrollment token: %w", err)
	}
	return token, nil
}
...
func (s *Store) EnrollTokenAccount(ctx context.Context, token string) (*Account, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("auth: reading enrollment token: %w", err)
	}
...
func (s *Store) ConsumeEnrollToken(ctx context.Context, token string) (*Account, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("auth: consuming enrollment token: %w", err)
	}
	return s.AccountByID(ctx, accountID)
}
...
func (s *Store) SessionAccount(ctx context.Context, token string, ttl, slideAfter time.Duration) (*Account, time.Time, error) {
	...
	if err != nil {
		return nil, time.Time{}, fmt.Errorf("auth: reading session: %w", err)
	}
...
func (s *Store) DeleteSessions(ctx context.Context, accountID int64) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("auth: deleting sessions: %w", err)
	}
	return tag.RowsAffected(), nil
}
...
func (s *Store) SaveChallenge(ctx context.Context, c Challenge, ttl time.Duration) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("auth: encoding challenge: %w", err)
	}
	...
	if _, err := s.pool.Exec(ctx, `
		...
		return "", fmt.Errorf("auth: saving challenge: %w", err)
	}
	return id, nil
}
...
func (s *Store) TakeChallenge(ctx context.Context, purpose, id string) (*Challenge, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("auth: taking challenge: %w", err)
	}
